...
public class Maximizar2R extends ProgramacaoLinear {
	...
	public double[] max2R(double[] Z, double[] r1, double[] r2) {
		...
		if (posicaoLinha == 0) {
			JOptionPane.showMessageDialog(null, "O problema é ilimitado ou não tem solução", "Ilimitado ou Sem solução",
	...
	private void verificarLoopTableau(double[] Z, double[] r1, double[] r2, double[] guardarInicialR1,
			double[] guardarInicialR2, double[] guardarInicialZ) {
		if (cont > 0) {
	...
	private double[] adicionarVariaveis(int tamanho, int restri) {
		...
		for (int i = 0; i < tamanho; i++) {
			...
			if (cont == tamanho) {
				aux = JOptionPane.showInputDialog(null,
						"Para a restrição " + restri + ", digite o valor do termo independente b" + restri);
